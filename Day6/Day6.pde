...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2019\\Day6\\data\\mydata");
...
HashMap<String, OrbitalBody> solarSystem = new HashMap<String, OrbitalBody>();
...
void setup() {
  ...
  {
    println(input.lines.get(i));
    tempStr=input.lines.get(i).split("\\x29");
    
    planet=solarSystem.get(tempStr[0]);
    if (planet==null)
    {
      planet=new OrbitalBody(tempStr[0]);
      solarSystem.put(tempStr[0],planet);
    }
    else
    {
      // clash - error?
      println("Planet clash found ["+tempStr[0]+"]");
    }

    moon=solarSystem.get(tempStr[1]);
    if (moon==null)
    {
      moon=new OrbitalBody(tempStr[1]);
      solarSystem.put(tempStr[1],moon);
    }

    planet.satellites.add(moon);
    moon.orbitsName=planet.name;
  }
  
  walkTree(0,"COM");
  
  int total=0;
  for (Map.Entry m : solarSystem.entrySet())
  {
    planet=(OrbitalBody)m.getValue();
    planet.printBody();
    total+=planet.orbitalDepth;
    println();
  }
  ...
  println();
  println(findPath("YOU"));
  println(findPath("SAN"));
  
  String[] t1;
  String[] t2;
  int u1=0,u2=0;
  t1=findPath("YOU").split(",");
  t2=findPath("SAN").split(",");
  
  for (i=0;t1[i].equals(t2[i])==true;i++)
  {
    print(t1[i]+"-");
  }
  println();
  println("Common path: "+i);
  u1=t1.length-i;u1--;
  u2=t2.length-i;u2--;
  println("Uncommon:"+u1+" "+u2);
  println("Total:"+(u1+u2));
}

String findPath(String t)
{
  String hunt=t;
  String path="";
  int i=0;
  ...
  while (hunt.equals("COM")==false)
  {
    path=hunt+","+path;
    planet=solarSystem.get(hunt);
    //planet.printBody();
    //println();
    hunt=planet.orbitsName;
    i++;
  }
  return(path);
}

void walkTree(int d, String n)
{
  OrbitalBody b;
  int i;
  ...
  if (b!=null)
  {
    ...
    //}
    println("D="+d+" ["+n+"]");
    if (b.satellites.size()>0)
    {     
      for (i=0;i<b.satellites.size();i++)
      {
        walkTree(d+1,b.satellites.get(i).name);
      }
    }
  }
}
...
{
  ArrayList<OrbitalBody> satellites = new ArrayList<OrbitalBody>();
  String name;
  String orbitsName;
  int orbitalDepth=0;
  
  public OrbitalBody(String n)
  {
    name=n;
  }
